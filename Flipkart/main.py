...
PRICE_CLASS = 'Nx9bqj CxhGGd'  # Class for extracting price
...
# Function to extract available sizes
def extract_available_sizes(soup):
    # Extract Available Sizes
    available_sizes = [li.find('a').text for li in soup.find_all('li', class_='aJWdJI')]

    # Define All Possible Sizes (XS to XXL)
    all_sizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL']

    # Create a dictionary to store size availability
    size_availability = {size: 'Yes' if size in available_sizes else '#N/A' for size in all_sizes}
...
def extract_price(soup):
    ...
    if price_div:
        price = price_div.get_text(strip=True).replace('₹', '')  # Remove currency symbol
        return price
    return "#N/A"  # Return default if no price found
...
def extract_ratings_reviews_product_rating(url, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            # Extract available sizes
            size_availability = extract_available_sizes(soup)

            # Extract price
            price = extract_price(soup)

            # Log the extracted data
            logging.info(f"Extracted Data for {url}: Ratings={ratings}, Reviews={reviews}, Product Rating={product_rating}, Brand={brand}, Price={price}, Sizes={size_availability}")

            return ratings, reviews, product_rating, brand, price, size_availability

        except (requests.RequestException, ValueError) as e:
            logging.error(f"Error fetching data from {url} on attempt {attempt + 1}: {e}")
            time.sleep(2 ** attempt)  # Exponential backoff for retries

    return "0", "0", "0", None, "#N/A", None  # Return default values if all attempts fail
...
def scrape_and_save_to_csv(input_csv, output_csv):
    ...
    try:
        with open(input_csv, mode='r', newline='', encoding='utf-8') as infile, \
            ...
            if outfile.tell() == 0:
                writer.writerow(['URL', 'Ratings', 'Reviews', 'Product Rating', 'Brand', 'Price', 'XS', 'S', 'M', 'L', 'XL', 'XXL'])
            ...
            for row in tqdm(urls, desc="Scraping Progress", unit="url"):
                ...
                if url:
                    ...
                    ratings, reviews, product_rating, brand, price, size_availability = extract_ratings_reviews_product_rating(url)
                    writer.writerow([
                        url, ratings, reviews, product_rating, brand or "", price,
                        size_availability['XS'], size_availability['S'], size_availability['M'],
                        size_availability['L'], size_availability['XL'], size_availability['XXL']
                    ])
